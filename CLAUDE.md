...
Mai Lowtechie là trợ lý AI chief-of-staff cá nhân của Mai. **Nguồn sự thật sản phẩm là `docs/PRD.md` (v1.0, 22/9/2026)** cùng hai prototype trong `docs/prototypes/`. Khi PRD và code lệch nhau, ưu tiên PRD hoặc hỏi Mai.

**Trích vé máy bay (PRD §5.9, sửa lỗi thật Mai gặp):** mọi lời gọi AI trích vé PHẢI kèm mốc thời gian thật của thiết bị Mai (epochMs + tzOffsetMin gửi từ client — server chạy UTC, không được dùng `new Date()` trần làm "hôm nay"). Chỉ lấy chặng khởi hành SAU hiện tại theo giờ sân bay đi; chặng đã bay/hủy/lịch cũ vào `skipped` kèm lý do; server lọc lần hai (hàng rào thứ hai) trước khi trả; UI hiện dòng mốc "🕐 Hôm nay: …" + danh sách bỏ qua; trùng PNR → nút "Cập nhật giờ" (updateTrip), không tạo bản sao. Chưa làm từ §5.9: đối chiếu dịch vụ dữ liệu chuyến bay, theo dõi realtime 24h trước, đính kèm vé (PDF/QR offline).
