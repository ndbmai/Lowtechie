...
**Nhận dạng loại ảnh trước khi trích** (lỗi thấy 23/9: gửi ảnh banner sự kiện của The Circle, app trả lời "Mình không đọc được dòng việc nào trong ảnh")
- Nguyên nhân: app chỉ có một đường xử lý, đi tìm **dòng việc kiểu checklist**; ảnh banner không có dòng việc nào nên trả về rỗng.
- **Bước 1 — phân loại ảnh:** checklist / ghi chú viết tay · **banner, poster, thiệp mời sự kiện** · ảnh chụp màn hình chat hoặc email · tài liệu, hợp đồng · danh thiếp · ảnh khác. Phân loại xong mới chọn cách trích tương ứng.
- **Bước 2 — trích theo loại:** banner → tạo sự kiện (bên dưới); chat/email → trích việc kèm người và hạn; tài liệu → trích việc và mốc quan trọng; danh thiếp → tạo liên hệ, gợi ý thêm vào danh bạ khách hàng của một dự án.
- **Không bao giờ trả lời cụt.** Nếu không chắc loại ảnh hoặc đọc được ít, Lowtechie hiện những gì đã đọc được và hỏi một câu: *"Ảnh này là sự kiện, danh sách việc, hay ghi chú?"*, kèm nút tạo việc / tạo sự kiện thủ công.
- **Ảnh mờ, nhỏ, chụp lại màn hình:** báo rõ lý do (độ phân giải thấp, chữ quá nhỏ, lóa) và đề nghị gửi ảnh gốc hoặc chụp lại gần hơn, thay vì chỉ nói "không đọc được".
- Test bắt buộc: ảnh banner sự kiện (ảnh gốc và ảnh chụp lại màn hình), ảnh checklist viết tay, ảnh chụp màn hình chat, ảnh danh thiếp.
...
- **Gắn dự án:** hỏi hoặc đoán theo nội dung (sự kiện do The Circle tổ chức hoặc có logo Circle → Circle; workshop học → Học tập), Mai đổi một chạm trên thẻ xem trước.
- **Lịch đích tự chọn theo dự án:** sự kiện của Circle → **lịch The Circle trên Lark**; Sorene → mai@sorene.ai; Cá nhân, Học tập → lịch cá nhân. Thẻ xem trước hiện sẵn lịch đích đó, Mai chỉ cần bấm Book (quy tắc đặt ở 5.3.4).
- **Tùy chọn book thẳng:** Mai bật được "sự kiện đọc từ banner, đủ ngày giờ và địa điểm → book thẳng vào lịch đích, báo sau" cho từng dự án. Mặc định vẫn hỏi; sự kiện có mời người khác thì luôn hỏi.
...
### 5.3 Dự án
- Mỗi dự án có: thành viên, kênh chat/email liên kết, khách hàng/đối tác, file liên kết, decision log.
...
- **Nút micro ngay trong ô:** Mai nói một câu ("ký lại hợp đồng website cho Đô Thị, hạn thứ Tư") → các trường tự điền, Mai xem lại rồi lưu.
- **Thiếu trường phân loại thì hỏi lại:** nếu sau khi gõ/nói mà còn thiếu category, khách hàng hoặc deadline, Lowtechie hỏi ngắn ngay trong ô, **mỗi lần một câu**, kèm 2–3 lựa chọn bấm nhanh (ví dụ: *"Việc này thuộc category nào?"* → Khách hàng · Đào tạo · Marketing). Mai trả lời bằng bấm, gõ hoặc nói.
- Mai chọn "Bỏ qua" cho từng câu hỏi; việc vẫn lưu được với trường trống (khách hàng trống → vào nhóm "Chưa gắn khách").
- Thêm nhiều việc liên tiếp: lưu xong ô trống lại, giữ nguyên category vừa dùng.

**Cách xem và bộ lọc: hai dropdown, mỗi cái một dòng**
- **Dropdown 1 — Nhóm theo:** Category (mặc định) · Khách hàng · Hạn. Thay cho ba nút tab to trước đây.
- **Dropdown 2 — Lọc:** Tất cả (mặc định) · Có hạn · Không hạn · Ưu tiên cao · Của cộng sự · Quá hạn.
- Mỗi dropdown là **một dòng duy nhất**, chữ không xuống dòng, hiện lựa chọn đang chọn; hai dropdown nằm cùng một hàng nếu đủ chỗ. Không dùng hàng chip nhiều dòng chiếm hết màn hình.
- Lựa chọn được nhớ cho lần mở sau.

**Các nhóm trong danh sách**
- Nhóm theo Category: mỗi category là một nhóm thu gọn/mở rộng được, có số việc; cuối nhóm có "Đã xong (N)".
- Nhóm theo Khách hàng: mỗi khách một nhóm; chạm tên khách → màn chi tiết khách hàng (5.3.2).
- **Nhóm "Chưa gắn khách (N)":** chạm vào tiêu đề nhóm → hiện danh sách khách của dự án để gắn nhanh cho **cả nhóm**; chạm vào một việc trong nhóm → chọn khách cho riêng việc đó (một chạm, không cần mở màn chi tiết). Có cả dòng "Tạo khách mới".
...
- Mỗi dự án: tên, màu, Tạm ngưng, Xóa, danh sách category, **danh sách khách hàng & đối tác**.
- **Bỏ ô mô tả dự án.**
- **Bỏ ô "h/tuần"**.
...
**Với dự án**
- Thêm mới: tên, màu, biểu tượng, thành viên, kênh chat/email liên kết, từ khóa nhận diện.
...
**Nhập lịch về app (đọc), không chỉ ghi ra** (lỗi thấy 23/9: đã kết nối Lark nhưng sự kiện trên Lark Calendar không hiện trong app)
- Kết nối xong phải chạy **nhập lần đầu** ngay: lấy toàn bộ sự kiện trong khoảng mặc định (12 tháng trước → 12 tháng sau), các tháng ngoài khoảng tải dần khi Mai cuộn tới.
- Với mỗi tài khoản: **lấy danh sách lịch trước** (lịch chính, lịch nhóm, lịch đã đăng ký), rồi lấy sự kiện của từng lịch đang bật — không giả định chỉ có một lịch chính.
- **Đồng bộ tiếp diễn:** nhận thay đổi qua webhook/subscription nếu nhà cung cấp hỗ trợ, cộng thêm đồng bộ định kỳ (ví dụ mỗi 15 phút) và khi Mai mở màn Lịch.
- **Màn trạng thái đồng bộ** (trong 5.3.4): mỗi tài khoản hiện lần đồng bộ gần nhất, **số lịch con và số sự kiện đã nhập**, nút **Đồng bộ ngay**. Nếu số sự kiện bằng 0, đây là dấu hiệu lỗi rõ ràng.
- **Lỗi phải nói rõ và có việc để làm**, ví dụ: "Thiếu quyền đọc lịch — cần admin Lark duyệt", "Phiên đăng nhập hết hạn — kết nối lại", "Chưa chọn lịch con nào". Không im lặng hiện lịch trống.
- Những chỗ cần kiểm tra khi lỗi này xảy ra: quyền đọc lịch đã được cấp và được admin duyệt chưa; dùng **user access token** (lịch cá nhân) thay vì chỉ tenant token; đã gọi API **danh sách lịch** rồi mới gọi API sự kiện chưa; khoảng thời gian truyền vào có đúng định dạng và có bao phủ hôm nay không; đã theo phân trang (page token) đến hết chưa; đúng miền (larksuite quốc tế hay feishu) chưa.
- Sự kiện nhập về hiện kèm dấu cho biết thuộc tài khoản nào, giữ nguyên múi giờ gốc, và tính vào kiểm tra bận/rảnh.
- Test: kết nối một tài khoản Lark có sẵn sự kiện → trong vòng 1 phút app phải hiện đúng số sự kiện của tháng hiện tại, khớp với app Lark.
